...
inline
trajectory_tracer::trajectory_tracer(const domain_t& d)
: rand_colors_(d),
  display_image_(d),
  traj_heads_(d),
  traj_(d),
...
__global__ void update_trajectories(kernel_image2d<i_float4> rand_colors,
				    ...
				    kernel_image2d<trajectory_tracer::trace> new_traces,
				    ...
				    kernel_image2d<short> age)
{
  ...
  new_traces(p).color = i_float4(0.f, 0.f, 0.f, 1.f);
...
template <typename X>
__global__ void draw_traj_heads(kernel_image2d<trajectory_tracer::trace> traces,
...
inline
void
trajectory_tracer::update(const image2d<i_short2>& matches,
                          const image2d<P>& particles)
{
  ...
  update_trajectories<P><<<dimgrid, dimblock>>>
...
inline
void
trajectory_tracer::display(const std::string& w, const image2d<i_float1>& colors)
{
  dim3 dimblock(16, 16);
  dim3 dimgrid = grid_dimension(rand_colors_.domain(), dimblock);

  image2d<i_float4> tmp(colors.domain());
