...
// #endif
  ...
  */
  ...
  inline
  void
  ofast382sl_feature::update(const image2d<i_float4>& in)
  {
  ...
  inline
  void
  ofast382sl_feature::update(const image2d<i_float1>& in)
  {
    swap_buffers();
                                             ...
                                             const point2d<int>& n)
  {
    ...
    d += ::abs(float(a.state[0]) - blurred_s1_(i_int2(n) + i_int2(circle_r3[a.o1])).x * 255);
    d += ::abs(float(a.state[1]) - blurred_s1_(i_int2(n) + i_int2(circle_r3[a.o1+8])).x * 255);

    i_int2 delta = i_int2(circle_r3[a.o2][0], circle_r3[a.o2][1]) * 2;
    d += ::abs(float(a.state[2]) - blurred_s2_(i_int2(n) + delta).x * 255);
    delta = i_int2(circle_r3[a.o2 + 8][0], circle_r3[a.o2 + 8][1]) * 2;
    d += ::abs(float(a.state[3]) - blurred_s2_(i_int2(n) + delta).x * 255);
  ...
  kernel_ofast382sl_feature::new_state(const point2d<int>& n)
  {
    ...
    i_int2 delta = i_int2(circle_r3[a.o2][0], circle_r3[a.o2][1]) * 2;
    res.state[2] = blurred_s2_(i_int2(n) + delta).x * 255;
    delta = i_int2(circle_r3[a.o2 + 8][0], circle_r3[a.o2 + 8][1]) * 2;
