...
namespace cuimg
{
  ...
  {
    return (1.f / (sqrt(2.f * CUIMG_PI) * sigma))
  ...
  {
    return -(x / (sqrt(2.f * CUIMG_PI) * std::pow(sigma, 3.f)))
      * expf(-(x * x) / (2.f * sigma * sigma));
  }
  ...
  float gaussian_derivative<2>(float sigma, int x)
  {
    return (((x - sigma) * (x + sigma)) /
             (sqrt(2 * CUIMG_PI) * std::pow(sigma, 5.f)))
      * expf(-(x * x) / (2.f * sigma * sigma));
  }
  ...
  float gaussian_derivative<3>(float sigma, int x)
  {
    return ((x * (x * x - 3 * sigma * sigma)) /
             (sqrt(2 * CUIMG_PI) * std::pow(sigma, 7.f)))
      * expf(-(x * x) / (2.f * sigma * sigma));
  }
  ...
  float gaussian_derivative<4>(float sigma, int x)
  {
    return ((x*x*x*x - 6.f * x*x * sigma * sigma + 3 * std::pow(sigma, 4.f)) /
             (sqrt(2.f * CUIMG_PI) * std::pow(sigma, 9.f)))
  ...
  template <typename W>
  void make_gaussian_kernel_1d(int N, float sigma, std::vector<W>& out)
  {
  ...
  template <typename W>
  void make_gaussian_kernel_1d(int N, float sigma, std::vector<W>& out, unsigned size)
  {
    out.resize(size);
    int x = - int(size / 2);
