...
namespace cuimg
{
  ...
  //   float d = 0;
  //   for (char i = 0; i < 4; i++)
  //     d += (::abs(float(a[i]) - float(b[i])) + ::abs(float(a[i+4]) - float(b[i+4]))) *
  //       ::abs(float(a[i]) - float(a[i+4]));

  //   for (char i = 8; i < 12; i++)
  //     d += (::abs(float(a[i]) - float(b[i])) + ::abs(float(a[i+4]) - float(b[i+4]))) *
  //       ::abs(float(a[i]) - float(a[i+4]));

  //   return d / (255.f * 255.f * 16.f);
  // }
  ...
  */
                           ...
                           float grad_thresh)
  {
...

    {
      ...
      for(unsigned i = 0; i < 16; i+=2)
      {
        ...
        if (frame_s1.has(n1))
          ...
          distances[i/2+8] = (frame_s2(n1) * 255);
        else
          distances[i/2+8] = 0.f;
      }
    }

    {
      ...
      for(int i = 0; i < 12; i++)
      {
        ...
        //                  p.col() + 1*circle_r3[i][1],
        //                  p.row() + 1*circle_r3[i][0]
        //   );

        // float v2 = tex2D(s1_tex,
        //                  p.col() + 1*circle_r3[(i+8)][1],
        //                  p.row() + 1*circle_r3[(i+8)][0]);

        float v1 = tex2D(s1_tex,
                         p.col() + 1*circle_r3_nb[i][1],
                         p.row() + 1*circle_r3_nb[i][0]
          );

        float v2 = tex2D(s1_tex,
                         p.col() + 1*circle_r3_nb[(i+12)][1],
                         p.row() + 1*circle_r3_nb[(i+12)][0]);
...

        {
          ...
          if (adiff < min_diff)
          {
            ...
            {
              float sv1 = tex2D(s1_tex,
                               p.col() + (j/3.f)*circle_r3_nb[i][1],
                               p.row() + (j/3.f)*circle_r3_nb[i][0]
                               );

              float sv2 = tex2D(s1_tex,
                                p.col() + (j/3.f)*circle_r3_nb[(i+12)][1],
                                p.row() + (j/3.f)*circle_r3_nb[(i+12)][0]);
  ...
  inline
  void
  fast382sl_feature::update(const image2d_f4& in)
  {
  ...
  inline
  void
  fast382sl_feature::update(const image2d_f1& in)
  {
    swap_buffers();
  ...
  inline
  __device__ float kernel_fast382sl_feature::distance_linear(const dfast382sl& a,
                                                             const point2d<int>& n)
  {
