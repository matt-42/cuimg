...
#define benchmark_matcher_kernel_sig(TG, F, T)    \
  i_short2* ,                                   \
    unsigned ,                                  \
    F,                                          \
    kernel_image2d<T>,                          \
...
#pragma unroll 4
  ...
  benchmark_matcher<F, HS>::benchmark_matcher(const domain_t& d)
    : t1_(d),
      t2_(d),
      distance_(d),
      test_(d),
      ...
      frame_cpt(0)
  {
   ...
   template <typename F, unsigned HS>
  void
  benchmark_matcher<F, HS>::swap_buffers()
  {
...
//    fill(*new_particles_, p);
  ...
  template <typename F, unsigned HS>
  unsigned
  benchmark_matcher<F, HS>::n_particles() const
  {
    return n_particles_;
  }
  ...
  template <typename F, unsigned HS>
  void
  benchmark_matcher<F, HS>::update(F& f, global_mvt_thread<benchmark_matcher<F, HS> >& t_mvt,
  ...
  template <typename F, unsigned HS>
  void
  benchmark_matcher<F, HS>::update(const flag<GPU>&, F& f, global_mvt_thread<benchmark_matcher<F, HS> >& t_mvt,
                                 const image2d_s2& ls_matches)
  {
    frame_cpt++;
    dim3 dimblock(128, 1, 1);
    ...
    if (n_particles_ > 0)
    {
      ...
      pw_call<benchmark_matcher_kernel_sig(GPU, typename F::kernel_type, particle)>
...
# endif
  ...
  template <typename F, unsigned HS>
  void
  benchmark_matcher<F, HS>::update(const flag<CPU>&, F& f, global_mvt_thread<benchmark_matcher<F, HS> >& t_mvt,
                                 const image2d_s2& ls_matches)
  {
    frame_cpt++;
...
#endif
      i_short2 mvt = t_mvt.mvt() * 2;
      pw_call<benchmark_matcher_kernel_sig(CPU, typename F::kernel_type, particle)>
...
# endif
  ...
  template <typename F, unsigned HS>
  void
  benchmark_matcher<F, HS>::display() const
  {
#ifdef WITH_DISPLAY
  ...
  template <typename F, unsigned HS>
  const typename benchmark_matcher<F, HS>::image2d_P&
  benchmark_matcher<F, HS>::particles() const
  {
    return *new_particles_;
  }

  template <typename F, unsigned HS>
  typename benchmark_matcher<F, HS>::particle_vector&
  benchmark_matcher<F, HS>::compact_particles()
  {
    return compact_particles_;
  }

  template <typename F, unsigned HS>
  const typename benchmark_matcher<F, HS>::particle_vector&
  benchmark_matcher<F, HS>::compact_particles() const
  {
    return compact_particles_;
  }

  template <typename F, unsigned HS>
  const typename benchmark_matcher<F, HS>::image2d_s2&
  benchmark_matcher<F, HS>::matches() const
  {
    return matches_;
  }

  template <typename F, unsigned HS>
  const typename benchmark_matcher<F, HS>::image2d_c&
  benchmark_matcher<F, HS>::errors() const
  {
    return errors_;
  }
...
#endif // !  CUIMG_BENCHMARK_MATCHER_HPP_
