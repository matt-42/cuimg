#ifndef CUIMG_LIGHT_MATCHER_HPP_
...
#define ligth_matching_kernel_sig(TG, F, T)    \
  i_short2* ,                                   \
    unsigned ,                                  \
    F,                                          \
    kernel_image2d<T>,                          \
...
#pragma unroll 4
  ...
  // Constructor.
  template <typename F>
  light_matcher<F>::light_matcher(const domain_t& d)
    : t1_(d),
      t2_(d),
      distance_(d),
      test_(d),
...
//    fill(*new_particles_, p);
  ...
  template <typename F>
  unsigned
  light_matcher<F>::n_particles() const
  {
    return n_particles_;
  }
  ...
  template <typename F>
  void
  light_matcher<F>::update(F& f, global_mvt_thread<light_matcher<F> >& t_mvt,
  ...
  template <typename F>
  void
  light_matcher<F>::update(const flag<GPU>&, F& f, global_mvt_thread<light_matcher<F> >& t_mvt,
                                 const image2d_s2& ls_matches)
  {
    SCOPE_PROF(light_matcher_update);
    ...
    if (n_particles_ > 0)
    {
      ...
      pw_call<ligth_matching_kernel_sig(GPU, typename F::kernel_type, particle)>
...
# endif
                                 ...
                                 const image2d_s2& ls_matches)
  {
    ...
    if (n_particles_ > 0)
    {
      ...
      pw_call<ligth_matching_kernel_sig(CPU, typename F::kernel_type, particle)>
...
# endif
  ...
  template <typename F>
  void
  light_matcher<F>::particles_compation(const flag<CPU>&)
  {
...
#pragma omp for schedule(static, 1024)
  ...
  template <typename F>
  void
  light_matcher<F>::particles_compation_old(const flag<CPU>&)
  {
  ...
  template <typename F>
  const typename light_matcher<F>::image2d_P&
  light_matcher<F>::particles() const
  {
    return *new_particles_;
  }

  template <typename F>
  typename light_matcher<F>::particle_vector&
  light_matcher<F>::compact_particles()
  {
    return compact_particles_;
  }

  template <typename F>
  const typename light_matcher<F>::particle_vector&
  light_matcher<F>::compact_particles() const
  {
    return compact_particles_;
  }

  template <typename F>
  const typename light_matcher<F>::image2d_s2&
  light_matcher<F>::matches() const
  {
    return matches_;
  }

  template <typename F>
  const typename light_matcher<F>::image2d_c&
  light_matcher<F>::errors() const
  {
    return errors_;
  }
...
#endif // !  CUIMG_LIGHT_MATCHER_HPP_
