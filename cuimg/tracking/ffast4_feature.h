#ifndef CUIMG_FFAST4_FEATURE_H_
# define CUIMG_FFAST4_FEATURE_H_
...
namespace cuimg
{
  ...
  struct dffast4
  {
    ...
    __host__ __device__ dffast4(const float2& o)
    {
      tex_float = o;
    }

    __host__ __device__ dffast4(const dffast4& o)
    {
    ...
    __host__ __device__
    dffast4& operator=(const dffast4& o)
    {
      //pertinence = o.pertinence;
      tex_float = o.tex_float;
      //for (unsigned i = 0; i < 16; i++) distances[i] = o.distances[i];
  ...
  template <typename V>
  class kernel_ffast4_feature;

  template <typename V, unsigned T>
  class ffast4_feature
  {
  public:
    enum { target = T };
    ...
    typedef dffast4 feature_t;
    typedef image2d_target(target, feature_t) image2d_D;

    typedef obox2d domain_t;

    typedef kernel_ffast4_feature<V> kernel_type; // client side GPU type.

    inline ffast4_feature(const domain_t& d);
  ...
  public:
    ...
    __host__ __device__
    inline kernel_ffast4_feature(ffast4_feature<V, target>& f);
    ...
    inline
    __host__ __device__ float distance_linear(const dffast4& a, const dffast4& b);

    inline
    __host__ __device__ float distance_linear(const dffast4& a,
					      const point2d<int>& n);
    ...
    inline
    __host__ __device__ float distance_linear_s2(const dffast4& a, const dffast4& b);
    inline
    __host__ __device__ float distance(const dffast4& a, const dffast4& b);
    inline
    __host__ __device__ float distance_s2(const dffast4& a, const dffast4& b);

    __host__ __device__ inline
    dffast4 weighted_mean(const dffast4& a, float aw, const point2d<int>& b, float bw);

    inline  __host__ __device__ dffast4
...
# include <cuimg/tracking/ffast4_feature.hpp>

#endif // ! CUIMG_FFAST4_FEATURE_H_
