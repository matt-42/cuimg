...
#pragma omp parallel for schedule(static, 300)
  ...
  template <typename F, void (F::*m)(int)>
  __global__ void
  run_kernel1d_kernel(const F& f_, unsigned size)
  {
  ...
  template <typename F>
  __global__ void
  run_kernel1d_functor_kernel(const F& f_, unsigned size)
  {
    unsigned i = thread_pos1d();
    if (i < size)
      f(i);
  ...
  {
    run_kernel1d<F, m><<<cuda_gpu::dimgrid1d(size), cuda_gpu::dimblock1d()>>>(f_, size);
  }

  template <typename F>
  void run_kernel1d_functor(const F& f_, unsigned size, const cuda_gpu&)
  {
    run_kernel1d<F><<<cuda_gpu::dimgrid1d(size), cuda_gpu::dimblock1d()>>>(f_, size);
  }
#endif
